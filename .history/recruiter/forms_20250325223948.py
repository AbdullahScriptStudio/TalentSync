...
class JobPostForm(forms.ModelForm):
    class Meta:
        model = JobPost
        fields = [
            'title', 'company', 'country', 'state_province', 'city', 'job_type', 'department',
    ...
    skills_required = forms.CharField(
        widget=forms.Textarea(attrs={'class': 'w-full p-3 text-sm border rounded-xl', 'placeholder': 'Enter skills separated by commas...'}),
        required=True,
        error_messages={'required': 'Skills are required!'}
    )