...
def chatbot_response(request):
    if request.method == "POST":
        try:
            ...
            # 🚨 Restriction: Define the chatbot's role & allowed topics
            system_instruction = (
                "You are a helpful assistant for a job recruitment platform. "
                "You should only answer questions related to job applications, hiring, interview tips, and career guidance. "
                "If asked about anything unrelated (e.g., weather, politics, random facts), politely refuse to answer."
            )

            # 🔹 Send structured input to Gemini AI
