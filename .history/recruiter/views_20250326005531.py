...
def job_edit(request, id):
    job = get_object_or_404(JobPost, id=id)  # Fetch the job by its ID

    if request.method == 'POST':
        form = JobPostForm(request.POST, instance=job)  # Pass the existing job instance to the form
        if form.is_valid():
        ...
        form = JobPostForm(instance=job)  # Pre-populate the form with the existing job details
