...
from people.models import CustomUser  # Importing the user model
...
def job_list(request):
    print(f"User: {request.user}, Role: {request.user.role}")  # Debugging

    if is_recruiter(request.user):
        job_posts = JobPost.objects.filter(recruiter=request.user).order_by('-posted_at')
        print(f"Recruiter Jobs Found: {job_posts.count()}")  # Debugging
    else:
