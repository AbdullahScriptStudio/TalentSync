...
class JobPost(models.Model):
    ...
    )  # Ensure job is linked to a recruiter

    title = models.CharField(max_length=255)
    country = models.CharField(max_length=2, default=COUNTRY)
    state_province = models.CharField(choices=GERMAN_STATES, max_length=100)
    city = models.CharField(choices=GERMAN_CITIES, max_length=100)
    ...
    salary_currency = models.CharField(choices=CURRENCY_CHOICES, max_length=3, default='EUR')
    skills_required = models.CharField(max_length=500)
    job_description = models.TextField()
    posted_at = models.DateTimeField(auto_now_add=True)
    status = models.CharField(choices=JOB_STATUS, max_length=10, default='open')

    def __str__(self):
        return f"{self.title} - {self.company} (Posted by {self.recruiter.username})"
