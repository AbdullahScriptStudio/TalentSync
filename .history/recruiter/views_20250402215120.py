...
def get_recruiter_context(user):
    ...
    profile_picture = user.profile_pic.url if user.profile_pic else staticfiles_storage.url('images/profile.png')
    return {
...
def custom_404(request, exception):
    return render(request, 'recruiter/error.html', status=404)

def custom_500(request):
    return render(request, 'recruiter/error.html', status=500)