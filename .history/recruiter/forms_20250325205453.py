...
class JobPostFormStep2(forms.Form):
    ...
    country = forms.ChoiceField(
        choices=[('DE', 'Germany')],
        required=True,
        error_messages={'required': 'Country selection is required!'}
    )
    state_province = forms.ChoiceField(
        choices=[('Bavaria', 'Bavaria'), ('Berlin', 'Berlin')],
        required=True,
        error_messages={'required': 'State/Province is required!'}
    )
    city = forms.ChoiceField(
        choices=[('Munich', 'Munich'), ('Berlin', 'Berlin')],
        required=True,
        error_messages={'required': 'City selection is required!'}
    )
    
    job_type = forms.ChoiceField(
        choices=JOB_TYPES,
        required=True,
        error_messages={'required': 'Job type is required!'}
    )
    department = forms.ChoiceField(
        choices=[('Human Resources', 'Human Resources'), 
                 ('Engineering', 'Engineering'),
                 ('Marketing', 'Marketing'),
                 ('Sales', 'Sales')],
        required=True,
        error_messages={'required': 'Department selection is required!'}
    )
    
    min_experience = forms.IntegerField(
        required=True,
        error_messages={'required': 'Minimum experience is required!'}
    )
    
    max_experience = forms.IntegerField(
        required=True,
        error_messages={'required': 'Maximum experience is required!'}
    )
    
    min_salary = forms.DecimalField(
        max_digits=10, decimal_places=2,
        required=True,
        error_messages={'required': 'Minimum salary is required!'}
    )
    
    max_salary = forms.DecimalField(
        max_digits=10, decimal_places=2,
        required=True,
        error_messages={'required': 'Maximum salary is required!'}
    )
    
    salary_currency = forms.ChoiceField(
        choices=[('USD', 'USD'), ('EUR', 'EUR'), ('GBP', 'GBP')],
        required=True,
        error_messages={'required': 'Salary currency is required!'}
    )
    
    skills_required = forms.CharField(
        max_length=500, required=False,
        error_messages={'required': 'Skills required field cannot be empty!'}
    )
    
    job_status = forms.ChoiceField(
        choices=JOB_STATUS,
        required=True,
        error_messages={'required': 'Job status is required!'}
    )
