...
def job_create_step2(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session['skills_required'] = ','.join(form.cleaned_data['skills_required'])  # Save skills as comma-separated values
            request.session['job_status'] = form.cleaned_data['job_status']
            request.session['is_remote'] = form.cleaned_data['is_remote']
            
            # Validation for min/max salary and experience
            ...
            # Once all data is validated, save the JobPost model
            job_post = JobPost(
                ...
                job_status=request.session.get('job_status'),
                is_remote=request.session.get('is_remote')
            )
            job_post.save()
        ...
        form = JobPostFormStep2(initial={
            ...
            'skills_required': request.session.get('skills_required', '').split(','),
