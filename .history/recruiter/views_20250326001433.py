...
def job_create_step3(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('job_list')  # Adjust this to your success/summary page URL
    else:
        form = JobPostFormStep3()

    return render(request, 'recruiter/job/create/', {'form': form})
...
def job_create_combined(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            job_post.save()
            return redirect('success_page')  # Redirect after saving the job post
    else:
        form = JobPostForm()
