...
def job_create_step2(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session['skills_required'] = form.cleaned_data['skills_required']  # Store skills as comma-separated values
            ...
            if form.cleaned_data['min_experience'] > form.cleaned_data['max_experience']:
                form.add_error('min_experience', 'Min experience cannot be greater than max experience')
                return render(request, 'recruiter/job_form_step2.html', {'form': form})

            if form.cleaned_data['min_salary'] > form.cleaned_data['max_salary']:
                form.add_error('min_salary', 'Min salary cannot be greater than max salary')
                return render(request, 'recruiter/job_form_step2.html', {'form': form})
            ...
            job_post = JobPost(
                ...
                job_type=request.session.get('job_type'),
                department=request.session.get('department'),
                min_experience=request.session.get('min_experience'),
                max_experience=request.session.get('max_experience'),
                min_salary=request.session.get('min_salary'),
                max_salary=request.session.get('max_salary'),
                salary_currency=request.session.get('salary_currency'),
                skills_required=request.session.get('skills_required'),
                job_status=request.session.get('job_status', 'open'),  # Default to 'open'
                is_remote=request.session.get('is_remote', False)
            )
            job_post.save()

            return redirect('job_create_step3')  # Go to step 3
        else:
            return render(request, 'recruiter/job_form_step2.html', {'form': form})
    else:
...
def job_create_step3(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Skills were collected as a comma-separated string
            skills_required = request.session.get('skills_required', '')
            job_description = form.cleaned_data['job_description']
            job_status = request.session.get('job_status', 'open')  # Collected in Step 2
            is_remote = request.POST.get('is_remote', False)  # From step 2 (checkbox)
            ...
            job_post = JobPost(
                ...
                skills_required=skills_required,  # Comma-separated skills string
                job_description=job_description,
                status=job_status,
                is_remote=is_remote  # Save the remote status
            )
            job_post.save()
