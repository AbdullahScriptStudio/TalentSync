...
def applicant_dashboard(request):
    ...
    context = {
        ...
        "applications": applications,  # This will be used to display applied jobs
...
def apply_on_job(request, job_id):
    ...
    return render(request, 'applicant/apply_on_job.html', context)
...
def applied_job_list(request):
    ...
    return render(request, "applicant/applied_job_list.html", context)



@login_required
def applied_job_description(request, application_id):
    # Get the application based on the provided application_id
    application = get_object_or_404(Application, id=application_id)

    # Ensure the logged-in user is the applicant for this application
    if application.applicant_email != request.user.email:
        return render(request, "error.html", {"message": "Access Denied"})

    # Get the job post related to this application
    ...
    return render(request, "applicant/applied_job_description.html", context)